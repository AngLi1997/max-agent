...
from app.services.system_menus import apply_menu_status

router = APIRouter(prefix="/menus", tags=["menus"])


def _menu_to_tree_item(menu: Menu) -> MenuTreeItem:
    return MenuTreeItem(
        id=menu.id,
        name=menu.name,
        path=menu.path,
        ...
        children=[_menu_to_tree_item(c) for c in menu.children],
    )
...
async def get_menu_tree(
    ...
    return [_menu_to_tree_item(m) for m in root_menus]
